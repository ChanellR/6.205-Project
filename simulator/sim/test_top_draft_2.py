...
async def top_test(dut):
    ...
    # await with_timeout(RisingEdge(dut.frame_done), 10*500, "ns")
    
    await FallingEdge(dut.is_density_task)
    dut._log.info("Frame done")
    
    val = dut.maccumulator.density_reciprocals.value
    values = [half((val & (0xFFFF << i * 16)) >> i * 16) for i in range(val.n_bits//16)]
    dut._log.info(f"Density_buffer: {values}")
    ...
    #         dut._log.info(f"step {i} position: {hex_position, float_position} -- velocity: {hex_velocity, float_velocity}")
        
    #     await wait(clk, 5)
    #     await FallingEdge(clk)
    #     set(dut, [("enable_stream",1)])
    #     await wait(clk, 4)
